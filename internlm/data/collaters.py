...
def packed_collate_fn_npu(batch, packed_length):
    ...
    for b in batch: # micro_num
        ...
        ), f"length of a sample should be equal to packed_length, but got {len(b['tokens'])},{len(b['tokens'][0])} and {packed_length})"
        assert (
    ...
    # print(f"attention_mask_micro_num_list: {len(attention_mask_micro_num_list)}", flush=True)
    # print(f"{len(xs)}, {len(xs[0])}")
    xs = torch.nn.utils.rnn.pad_sequence(xs, batch_first=True)
    ys = torch.nn.utils.rnn.pad_sequence(ys, batch_first=True, padding_value=-100)
    ts = torch.nn.utils.rnn.pad_sequence(ts, batch_first=True, padding_value=0)    
